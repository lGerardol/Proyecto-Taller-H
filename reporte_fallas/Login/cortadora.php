...
<head>
    ...
    <title>Tabla de Cortadora</title>
    <!-- Bootstrap CSS -->
...
<body>
    ...
    <div class="container mt-5">
        <h2 class="mb-4 text-center">Lista de Cortadora</h2>

        <!-- Botón para agregar una nueva cortadora -->
        <div class="text-right">
            <button class="btn btn-primary btn-add" onclick="mostrarFormularioAgregar()">
                <i class="fas fa-plus-circle"></i> Agregar nueva cortadora
            </button>
        </div>

        <!-- Formulario para agregar una nueva cortadora -->
        <div id="form-agregar">
            <h4 class="text-center">Agregar Nueva Cortadora</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-success" name="agregar_cortadora">Agregar Cortadora</button>
                <button type="button" class="btn btn-secondary" onclick="ocultarFormularioAgregar()">Cancelar</button>
            </form>
        </div>

        <!-- Formulario para editar una cortadora (inicialmente oculto) -->
        <div id="form-editar">
            <h4 class="text-center">Editar Cortadora</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-primary" name="actualizar_cortadora">Actualizar Cortadora</button>
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    if (isset($_POST['id_eliminar'])) {
                        ...
                        $sqlEliminar = "DELETE FROM cortadora WHERE id = ?";
                        $stmtEliminar = $conn->prepare($sqlEliminar);
                        $stmtEliminar->bind_param("i", $idEliminar);
                        
                        if ($stmtEliminar->execute()) {
                            echo "<div class='alert alert-success'>Cortadora eliminada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al eliminar la cortadora: " . $conn->error . "</div>";
                        }
                        $stmtEliminar->close();
                    }

                    // Procesar inserción de una nueva cortadora
                    if (isset($_POST['agregar_cortadora'])) {
                        ...
                        $sqlAgregar = "INSERT INTO cortadora (nombre, marca, modelo, codigo, fecha_ingreso, estado, observaciones, imagen_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                        ...
                        if ($stmtAgregar->execute()) {
                            echo "<div class='alert alert-success'>Nueva cortadora agregada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al agregar la cortadora: " . $conn->error . "</div>";
                        }
                        $stmtAgregar->close();
                    }

                    // Verificar si se ha enviado la actualización de una cortadora
                    if (isset($_POST['actualizar_cortadora'])) {
                        $id = $_POST['id'];
                        $nombre = $_POST['nombre'];
                        $marca = $_POST['marca'];
                        $modelo = $_POST['modelo'];
                        ...
                        $sqlActualizar = "UPDATE cortadora SET nombre=?, marca=?, modelo=?, codigo=?, fecha_ingreso=?, estado=?, observaciones=?, imagen_url=? WHERE id=?";
                        ...
                        if ($stmtActualizar->execute()) {
                            echo "<div class='alert alert-success'>Cortadora actualizada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al actualizar la cortadora: " . $conn->error . "</div>";
                        }
                        $stmtActualizar->close();
                    }

                    // Consulta SQL para obtener las cortadoras
                    $sql = "SELECT * FROM cortadora";
                    $result = $conn->query($sql);

                    // Mostrar las cortadoras en la tabla
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td><img src='" . $row["imagen_url"] . "' width='100' alt='Imagen de cortadora'></td>"; // Mostrar la imagen
                            echo "<td>";
                            ...
                            echo "<form method='POST' action='' class='d-inline' onsubmit='return confirm(\"¿Estás seguro que deseas eliminar esta cortadora?\");'>";
    ...
    <script>
        // Función para mostrar el formulario de agregar nueva cortadora
        ...
        // Función para ocultar el formulario de agregar nueva cortadora
        function ocultarFormularioAgregar() {
            document.getElementById("form-agregar").style.display = "none";
        }

        // Función para mostrar el formulario de edición con los datos de la cortadora seleccionada
