...
<body>
    <!-- Barra de navegación superior con el logo alineado a la izquierda -->
    <nav class="navbar" role="navigation" aria-label="main navigation">
        <div class="navbar-brand">
            <a class="navbar-item" href="#">
                <img src="https://www.itca.edu.sv/wp-content/uploads/2017/01/logo-new-slider-1.png" alt="Logo" class="logo-img">
            </a>
        </div>
    </nav>

    <div class="container mt-5">
        <h2 class="mb-4 text-center">Lista de Motores</h2>

        <div class="text-right">
            <button class="btn btn-primary btn-add" onclick="mostrarFormularioAgregar()">
                <i class="fas fa-plus-circle"></i> Agregar nuevo motor
            </button>
        </div>

        <div id="form-agregar">
            <h4 class="text-center">Agregar Nuevo Motor</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-success" name="agregar_motor">Agregar Motor</button>
                <button type="button" class="btn btn-secondary" onclick="ocultarFormularioAgregar()">Cancelar</button>
            </form>
        </div>

        <div id="form-editar">
            <h4 class="text-center">Editar Motor</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-primary" name="actualizar_motor">Actualizar Motor</button>
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    if (isset($_POST['id_eliminar'])) {
                        ...
                        $sqlEliminar = "DELETE FROM motores WHERE id = ?";
                        $stmtEliminar = $conn->prepare($sqlEliminar);
                        $stmtEliminar->bind_param("i", $idEliminar);

                        if ($stmtEliminar->execute()) {
                            echo "<div class='alert alert-success'>Motor eliminado correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al eliminar el motor: " . $conn->error . "</div>";
                        }
                        $stmtEliminar->close();
                    }

                    // Procesar inserción de un nuevo motor
                    if (isset($_POST['agregar_motor'])) {
                        ...
                        $sqlAgregar = "INSERT INTO motores (nombre, marca, modelo, codigo, fecha_ingreso, estado, observaciones, imagen_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                        ...
                        if ($stmtAgregar->execute()) {
                            echo "<div class='alert alert-success'>Nuevo motor agregado correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al agregar el motor: " . $conn->error . "</div>";
                        }
                        $stmtAgregar->close();
                    }

                    // Verificar si se ha enviado la actualización de un motor
                    if (isset($_POST['actualizar_motor'])) {
                        $id = $_POST['id'];
                        $nombre = $_POST['nombre'];
                        $marca = $_POST['marca'];
                        $modelo = $_POST['modelo'];
                        ...
                        $sqlActualizar = "UPDATE motores SET nombre=?, marca=?, modelo=?, codigo=?, fecha_ingreso=?, estado=?, observaciones=?, imagen_url=? WHERE id=?";
                        ...
                        if ($stmtActualizar->execute()) {
                            echo "<div class='alert alert-success'>Motor actualizado correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al actualizar el motor: " . $conn->error . "</div>";
                        }
                        $stmtActualizar->close();
                    }

                    // Consulta SQL para obtener los motores
                    $sql = "SELECT * FROM motores";
                    $result = $conn->query($sql);

                    // Mostrar los motores en la tabla
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td><img src='" . $row["imagen_url"] . "' width='100' alt='Imagen de motor'></td>"; // Mostrar la imagen
                            echo "<td>";
                            ...
                            echo "<form method='POST' action='' class='d-inline' onsubmit='return confirm(\"¿Estás seguro que deseas eliminar este motor?\");'>";
