...
<head>
    ...
    <title>Tabla de Guillotinas</title>
    <!-- Bootstrap CSS -->
    ...
    <style>
        ...
        /* Estilo para las barras horizontales */
        .navbar {
...
<body>
    ...
    <div class="container mt-5">
        <h2 class="mb-4 text-center">Lista de Guillotinas</h2>

        <!-- Botón para agregar una nueva guillotina -->
        <div class="text-right">
            <button class="btn btn-primary btn-add" onclick="mostrarFormularioAgregar()">
                <i class="fas fa-plus-circle"></i> Agregar nueva guillotina
            </button>
        </div>

        <!-- Formulario para agregar una nueva guillotina -->
        <div id="form-agregar">
            <h4 class="text-center">Agregar Nueva Guillotina</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-success" name="agregar_guillotina">Agregar Guillotina</button>
                <button type="button" class="btn btn-secondary" onclick="ocultarFormularioAgregar()">Cancelar</button>
            </form>
        </div>

        <!-- Formulario para editar una guillotina (inicialmente oculto) -->
        <div id="form-editar">
            <h4 class="text-center">Editar Guillotina</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-primary" name="actualizar_guillotina">Actualizar Guillotina</button>
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    if (isset($_POST['id_eliminar'])) {
                        ...
                        $sqlEliminar = "DELETE FROM guillotinas WHERE id = ?";
                        $stmtEliminar = $conn->prepare($sqlEliminar);
                        $stmtEliminar->bind_param("i", $idEliminar);
                        
                        if ($stmtEliminar->execute()) {
                            echo "<div class='alert alert-success'>Guillotina eliminada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al eliminar la guillotina: " . $conn->error . "</div>";
                        }
                        $stmtEliminar->close();
                    }

                    // Procesar inserción de una nueva guillotina
                    if (isset($_POST['agregar_guillotina'])) {
                        ...
                        $sqlAgregar = "INSERT INTO guillotinas (nombre, marca, modelo, codigo, fecha_ingreso, estado, observaciones, imagen_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                        ...
                        if ($stmtAgregar->execute()) {
                            echo "<div class='alert alert-success'>Nueva guillotina agregada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al agregar la guillotina: " . $conn->error . "</div>";
                        }
                        $stmtAgregar->close();
                    }

                    // Verificar si se ha enviado la actualización de una guillotina
                    if (isset($_POST['actualizar_guillotina'])) {
                        $id = $_POST['id'];
                        $nombre = $_POST['nombre'];
                        $marca = $_POST['marca'];
                        $modelo = $_POST['modelo'];
                        ...
                        $sqlActualizar = "UPDATE guillotinas SET nombre=?, marca=?, modelo=?, codigo=?, fecha_ingreso=?, estado=?, observaciones=?, imagen_url=? WHERE id=?";
                        ...
                        if ($stmtActualizar->execute()) {
                            echo "<div class='alert alert-success'>Guillotina actualizada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al actualizar la guillotina: " . $conn->error . "</div>";
                        }
                        $stmtActualizar->close();
                    }

                    // Consulta SQL para obtener las guillotinas
                    $sql = "SELECT * FROM guillotinas";
                    $result = $conn->query($sql);

                    // Mostrar las guillotinas en la tabla
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td><img src='" . $row["imagen_url"] . "' width='100' alt='Imagen de guillotina'></td>"; // Mostrar la imagen
                            echo "<td>";
                            ...
                            echo "<form method='POST' action='' class='d-inline' onsubmit='return confirm(\"¿Estás seguro que deseas eliminar esta guillotina?\");'>";
    ...
    <script>
        // Función para mostrar el formulario de agregar nueva guillotina
        ...
        // Función para ocultar el formulario de agregar nueva guillotina
        function ocultarFormularioAgregar() {
            document.getElementById("form-agregar").style.display = "none";
        }

        // Función para mostrar el formulario de edición con los datos de la guillotina seleccionada
