...
<head>
    ...
    <title>Tabla de Limadoras</title>
    <!-- Bootstrap CSS -->
...
<body>
    <!-- Barra de navegación superior con el logo a la izquierda -->
    <nav class="navbar">
        <div class="container">
    ...
    <div class="container mt-5">
        <h2 class="mb-4 text-center">Lista de Limadoras</h2>

        <!-- Botón para agregar una nueva limadora -->
        <div class="text-right">
            <button class="btn btn-primary btn-add" onclick="mostrarFormularioAgregar()">
                <i class="fas fa-plus-circle"></i> Agregar nueva limadora
            </button>
        </div>

        <!-- Formulario para agregar una nueva limadora -->
        <div id="form-agregar">
            <h4 class="text-center">Agregar Nueva Limadora</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-success" name="agregar_limadora">Agregar Limadora</button>
                <button type="button" class="btn btn-secondary" onclick="ocultarFormularioAgregar()">Cancelar</button>
            </form>
        </div>

        <!-- Formulario para editar una limadora (inicialmente oculto) -->
        <div id="form-editar">
            <h4 class="text-center">Editar Limadora</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-primary" name="actualizar_limadora">Actualizar Limadora</button>
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    if (isset($_POST['id_eliminar'])) {
                        ...
                        $sqlEliminar = "DELETE FROM limadoras WHERE id = ?";
                        $stmtEliminar = $conn->prepare($sqlEliminar);
                        $stmtEliminar->bind_param("i", $idEliminar);
                        
                        if ($stmtEliminar->execute()) {
                            echo "<div class='alert alert-success'>Limadora eliminada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al eliminar la limadora: " . $conn->error . "</div>";
                        }
                        $stmtEliminar->close();
                    }

                    // Procesar inserción de una nueva limadora
                    if (isset($_POST['agregar_limadora'])) {
                        ...
                        $sqlAgregar = "INSERT INTO limadoras (nombre, marca, modelo, codigo, fecha_ingreso, estado, observaciones, imagen_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                        ...
                        if ($stmtAgregar->execute()) {
                            echo "<div class='alert alert-success'>Nueva limadora agregada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al agregar la limadora: " . $conn->error . "</div>";
                        }
                        $stmtAgregar->close();
                    }

                    // Verificar si se ha enviado la actualización de una limadora
                    if (isset($_POST['actualizar_limadora'])) {
                        $id = $_POST['id'];
                        $nombre = $_POST['nombre'];
                        $marca = $_POST['marca'];
                        $modelo = $_POST['modelo'];
                        ...
                        $sqlActualizar = "UPDATE limadoras SET nombre=?, marca=?, modelo=?, codigo=?, fecha_ingreso=?, estado=?, observaciones=?, imagen_url=? WHERE id=?";
                        ...
                        if ($stmtActualizar->execute()) {
                            echo "<div class='alert alert-success'>Limadora actualizada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al actualizar la limadora: " . $conn->error . "</div>";
                        }
                        $stmtActualizar->close();
                    }

                    // Consulta SQL para obtener las limadoras
                    $sql = "SELECT * FROM limadoras";
                    $result = $conn->query($sql);

                    // Mostrar las limadoras en la tabla
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td><img src='" . $row["imagen_url"] . "' width='100' alt='Imagen de limadora'></td>"; // Mostrar la imagen
                            echo "<td>";
                            ...
                            echo "<form method='POST' action='' class='d-inline' onsubmit='return confirm(\"¿Estás seguro que deseas eliminar esta limadora?\");'>";
    ...
    <script>
        // Función para mostrar el formulario de agregar nueva limadora
        ...
        // Función para ocultar el formulario de agregar nueva limadora
        function ocultarFormularioAgregar() {
            document.getElementById("form-agregar").style.display = "none";
        }

        // Función para mostrar el formulario de edición con los datos de la limadora seleccionada
