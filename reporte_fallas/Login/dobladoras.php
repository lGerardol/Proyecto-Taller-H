...
<head>
    ...
    <title>Tabla de Dobladoras</title>
    <!-- Bootstrap CSS -->
...
<nav class="navbar" style="background-color: #dea632; padding: 10px;">
    <div class="container-fluid">
        <div class="navbar-brand">
            <a href="#">
                <img src="https://www.itca.edu.sv/wp-content/uploads/2017/01/logo-new-slider-1.png" alt="Logo" style="max-height: 40px;"> <!-- Ajuste de tamaño del logo -->
            </a>
        </div>
    </div>
</nav>
    ...
    <div class="container mt-5">
        <h2 class="mb-4 text-center">Lista de Dobladoras</h2>

        <!-- Botón para agregar una nueva dobladora -->
        <div class="text-right">
            <button class="btn btn-primary btn-add" onclick="mostrarFormularioAgregar()">
                <i class="fas fa-plus-circle"></i> Agregar nueva dobladora
            </button>
        </div>

        <!-- Formulario para agregar una nueva dobladora -->
        <div id="form-agregar">
            <h4 class="text-center">Agregar Nueva Dobladora</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-success" name="agregar_dobladora">Agregar Dobladora</button>
                <button type="button" class="btn btn-secondary" onclick="ocultarFormularioAgregar()">Cancelar</button>
            </form>
        </div>

        <!-- Formulario para editar una dobladora (inicialmente oculto) -->
        <div id="form-editar">
            <h4 class="text-center">Editar Dobladora</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-primary" name="actualizar_dobladora">Actualizar Dobladora</button>
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    if (isset($_POST['id_eliminar'])) {
                        ...
                        $sqlEliminar = "DELETE FROM dobladoras WHERE id = ?";
                        $stmtEliminar = $conn->prepare($sqlEliminar);
                        $stmtEliminar->bind_param("i", $idEliminar);
                        
                        if ($stmtEliminar->execute()) {
                            echo "<div class='alert alert-success'>Dobladora eliminada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al eliminar la dobladora: " . $conn->error . "</div>";
                        }
                        $stmtEliminar->close();
                    }

                    // Procesar inserción de una nueva dobladora
                    if (isset($_POST['agregar_dobladora'])) {
                        ...
                        $sqlAgregar = "INSERT INTO dobladoras (nombre, marca, modelo, codigo, fecha_ingreso, estado, observaciones, imagen_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                        ...
                        if ($stmtAgregar->execute()) {
                            echo "<div class='alert alert-success'>Nueva dobladora agregada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al agregar la dobladora: " . $conn->error . "</div>";
                        }
                        $stmtAgregar->close();
                    }

                    // Verificar si se ha enviado la actualización de una dobladora
                    if (isset($_POST['actualizar_dobladora'])) {
                        $id = $_POST['id'];
                        $nombre = $_POST['nombre'];
                        $marca = $_POST['marca'];
                        $modelo = $_POST['modelo'];
                        ...
                        $sqlActualizar = "UPDATE dobladoras SET nombre=?, marca=?, modelo=?, codigo=?, fecha_ingreso=?, estado=?, observaciones=?, imagen_url=? WHERE id=?";
                        ...
                        if ($stmtActualizar->execute()) {
                            echo "<div class='alert alert-success'>Dobladora actualizada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al actualizar la dobladora: " . $conn->error . "</div>";
                        }
                        $stmtActualizar->close();
                    }

                    // Consulta SQL para obtener las dobladoras
                    $sql = "SELECT * FROM dobladoras";
                    $result = $conn->query($sql);

                    // Mostrar las dobladoras en la tabla
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td><img src='" . $row["imagen_url"] . "' width='100' alt='Imagen de dobladora'></td>"; // Mostrar la imagen
                            echo "<td>";
                            ...
                            echo "<form method='POST' action='' class='d-inline' onsubmit='return confirm(\"¿Estás seguro que deseas eliminar esta dobladora?\");'>";
    ...
    <script>
        // Función para mostrar el formulario de agregar nueva dobladora
        ...
        // Función para ocultar el formulario de agregar nueva dobladora
        function ocultarFormularioAgregar() {
            document.getElementById("form-agregar").style.display = "none";
        }

        // Función para mostrar el formulario de edición con los datos de la dobladora seleccionada
