...
<head>
    ...
    <title>Tabla de Compresores</title>
    <!-- Bootstrap CSS -->
...
<body>
    ...
    <nav class="navbar" role="navigation" aria-label="main navigation">
        <div class="navbar-brand">
            <a class="navbar-item" href="#">
                <img src="https://www.itca.edu.sv/wp-content/uploads/2017/01/logo-new-slider-1.png" alt="Logo" width="150" height="40">
            </a>
        </div>
    </nav>

    <div class="container mt-5">
        <h2 class="mb-4 text-center">Lista de Compresores</h2>

        <!-- Botón para agregar un nuevo compresor -->
        <div class="text-right">
            <button class="btn btn-primary btn-add" onclick="mostrarFormularioAgregar()">
                <i class="fas fa-plus-circle"></i> Agregar nuevo compresor
            </button>
        </div>

        <!-- Formulario para agregar un nuevo compresor -->
        <div id="form-agregar">
            <h4 class="text-center">Agregar Nuevo Compresor</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-success" name="agregar_compresor">Agregar Compresor</button>
                <button type="button" class="btn btn-secondary" onclick="ocultarFormularioAgregar()">Cancelar</button>
            </form>
        </div>

        <!-- Formulario para editar un compresor (inicialmente oculto) -->
        <div id="form-editar">
            <h4 class="text-center">Editar Compresor</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-primary" name="actualizar_compresor">Actualizar Compresor</button>
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    if (isset($_POST['id_eliminar'])) {
                        ...
                        $sqlEliminar = "DELETE FROM compresores WHERE id = ?";
                        $stmtEliminar = $conn->prepare($sqlEliminar);
                        $stmtEliminar->bind_param("i", $idEliminar);
                        
                        if ($stmtEliminar->execute()) {
                            echo "<div class='alert alert-success'>Compresor eliminado correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al eliminar el compresor: " . $conn->error . "</div>";
                        }
                        $stmtEliminar->close();
                    }

                    // Procesar inserción de un nuevo compresor
                    if (isset($_POST['agregar_compresor'])) {
                        ...
                        $sqlAgregar = "INSERT INTO compresores (nombre, marca, modelo, codigo, fecha_ingreso, estado, observaciones, imagen_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                        ...
                        if ($stmtAgregar->execute()) {
                            echo "<div class='alert alert-success'>Nuevo compresor agregado correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al agregar el compresor: " . $conn->error . "</div>";
                        }
                        $stmtAgregar->close();
                    }

                    // Verificar si se ha enviado la actualización de un compresor
                    if (isset($_POST['actualizar_compresor'])) {
                        $id = $_POST['id'];
                        $nombre = $_POST['nombre'];
                        $marca = $_POST['marca'];
                        $modelo = $_POST['modelo'];
                        ...
                        $sqlActualizar = "UPDATE compresores SET nombre=?, marca=?, modelo=?, codigo=?, fecha_ingreso=?, estado=?, observaciones=?, imagen_url=? WHERE id=?";
                        ...
                        if ($stmtActualizar->execute()) {
                            echo "<div class='alert alert-success'>Compresor actualizado correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al actualizar el compresor: " . $conn->error . "</div>";
                        }
                        $stmtActualizar->close();
                    }

                    // Consulta SQL para obtener los compresores
                    $sql = "SELECT * FROM compresores";
                    $result = $conn->query($sql);

                    // Mostrar los compresores en la tabla
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td><img src='" . $row["imagen_url"] . "' width='100' alt='Imagen de compresor'></td>"; // Mostrar la imagen
                            echo "<td>";
                            ...
                            echo "<form method='POST' action='' class='d-inline' onsubmit='return confirm(\"¿Estás seguro que deseas eliminar este compresor?\");'>";
    ...
    <script>
        // Función para mostrar el formulario de agregar nuevo compresor
        ...
        // Función para ocultar el formulario de agregar nuevo compresor
        function ocultarFormularioAgregar() {
            document.getElementById("form-agregar").style.display = "none";
        }

        // Función para mostrar el formulario de edición con los datos del compresor seleccionado
