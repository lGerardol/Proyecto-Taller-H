...
<head>
    ...
    <style>
        ...
        .navbar-item img {
            ...
            padding-left: 15px; /* Logo completamente alineado a la izquierda */
        }
    </style>
</head>
<body>
    ...
    <div class="container mt-5">
        <h2 class="mb-4 text-center">Lista de Prensas</h2>

        <!-- Botón para agregar una nueva prensa -->
        <div class="text-right">
            <button class="btn btn-primary btn-add" onclick="mostrarFormularioAgregar()">
                <i class="fas fa-plus-circle"></i> Agregar nueva prensa
            </button>
        </div>

        <!-- Formulario para agregar una nueva prensa -->
        <div id="form-agregar">
            <h4 class="text-center">Agregar Nueva Prensa</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-success" name="agregar_prensa">Agregar Prensa</button>
                <button type="button" class="btn btn-secondary" onclick="ocultarFormularioAgregar()">Cancelar</button>
            </form>
        </div>

        <!-- Formulario para editar una prensa (inicialmente oculto) -->
        <div id="form-editar">
            <h4 class="text-center">Editar Prensa</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-primary" name="actualizar_prensa">Actualizar Prensa</button>
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    if (isset($_POST['id_eliminar'])) {
                        ...
                        $sqlEliminar = "DELETE FROM prensas WHERE id = ?";
                        $stmtEliminar = $conn->prepare($sqlEliminar);
                        $stmtEliminar->bind_param("i", $idEliminar);

                        if ($stmtEliminar->execute()) {
                            echo "<div class='alert alert-success'>Prensa eliminada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al eliminar la prensa: " . $conn->error . "</div>";
                        }
                        $stmtEliminar->close();
                    }

                    // Procesar inserción de una nueva prensa
                    if (isset($_POST['agregar_prensa'])) {
                        ...
                        $sqlAgregar = "INSERT INTO prensas (nombre, marca, modelo, codigo, fecha_ingreso, estado, observaciones, imagen_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                        ...
                        if ($stmtAgregar->execute()) {
                            echo "<div class='alert alert-success'>Nueva prensa agregada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al agregar la prensa: " . $conn->error . "</div>";
                        }
                        $stmtAgregar->close();
                    }

                    // Verificar si se ha enviado la actualización de una prensa
                    if (isset($_POST['actualizar_prensa'])) {
                        $id = $_POST['id'];
                        $nombre = $_POST['nombre'];
                        $marca = $_POST['marca'];
                        $modelo = $_POST['modelo'];
                        ...
                        $sqlActualizar = "UPDATE prensas SET nombre=?, marca=?, modelo=?, codigo=?, fecha_ingreso=?, estado=?, observaciones=?, imagen_url=? WHERE id=?";
                        ...
                        if ($stmtActualizar->execute()) {
                            echo "<div class='alert alert-success'>Prensa actualizada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al actualizar la prensa: " . $conn->error . "</div>";
                        }
                        $stmtActualizar->close();
                    }

                    // Consulta SQL para obtener las prensas
                    $sql = "SELECT * FROM prensas";
                    $result = $conn->query($sql);

                    // Mostrar las prensas en la tabla
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td><img src='" . $row["imagen_url"] . "' width='100' alt='Imagen de prensa'></td>"; // Mostrar la imagen
                            echo "<td>";
                            ...
                            echo "<form method='POST' action='' class='d-inline' onsubmit='return confirm(\"¿Estás seguro que deseas eliminar esta prensa?\");'>";
