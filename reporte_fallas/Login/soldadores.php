...
<head>
    ...
    <title>Tabla de Soldadores</title>
    <!-- Bootstrap CSS -->
...
<body>
    ...
    <div class="container mt-5">
        <h2 class="mb-4 text-center">Lista de Soldadores</h2>

        <!-- Botón para agregar un nuevo soldador -->
        <div class="text-right">
            <button class="btn btn-primary btn-add" onclick="mostrarFormularioAgregar()">
                <i class="fas fa-plus-circle"></i> Agregar nuevo soldador
            </button>
        </div>

        <!-- Formulario para agregar un nuevo soldador -->
        <div id="form-agregar">
            <h4 class="text-center">Agregar Nuevo Soldador</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-success" name="agregar_soldador">Agregar Soldador</button>
                <button type="button" class="btn btn-secondary" onclick="ocultarFormularioAgregar()">Cancelar</button>
            </form>
        </div>

        <!-- Formulario para editar un soldador (inicialmente oculto) -->
        <div id="form-editar">
            <h4 class="text-center">Editar Soldador</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-primary" name="actualizar_soldador">Actualizar Soldador</button>
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    if (isset($_POST['id_eliminar'])) {
                        ...
                        $sqlEliminar = "DELETE FROM soldadores WHERE id = ?";
                        $stmtEliminar = $conn->prepare($sqlEliminar);
                        $stmtEliminar->bind_param("i", $idEliminar);
                        
                        if ($stmtEliminar->execute()) {
                            echo "<div class='alert alert-success'>Soldador eliminado correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al eliminar el soldador: " . $conn->error . "</div>";
                        }
                        $stmtEliminar->close();
                    }

                    // Procesar inserción de un nuevo soldador
                    if (isset($_POST['agregar_soldador'])) {
                        ...
                        $sqlAgregar = "INSERT INTO soldadores (nombre, marca, modelo, codigo, fecha_ingreso, estado, observaciones, imagen_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                        ...
                        if ($stmtAgregar->execute()) {
                            echo "<div class='alert alert-success'>Nuevo soldador agregado correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al agregar el soldador: " . $conn->error . "</div>";
                        }
                        $stmtAgregar->close();
                    }

                    // Verificar si se ha enviado la actualización de un soldador
                    if (isset($_POST['actualizar_soldador'])) {
                        $id = $_POST['id'];
                        $nombre = $_POST['nombre'];
                        $marca = $_POST['marca'];
                        $modelo = $_POST['modelo'];
                        ...
                        $sqlActualizar = "UPDATE soldadores SET nombre=?, marca=?, modelo=?, codigo=?, fecha_ingreso=?, estado=?, observaciones=?, imagen_url=? WHERE id=?";
                        ...
                        if ($stmtActualizar->execute()) {
                            echo "<div class='alert alert-success'>Soldador actualizado correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al actualizar el soldador: " . $conn->error . "</div>";
                        }
                        $stmtActualizar->close();
                    }

                    // Consulta SQL para obtener los soldadores
                    $sql = "SELECT * FROM soldadores";
                    $result = $conn->query($sql);

                    // Mostrar los soldadores en la tabla
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td><img src='" . $row["imagen_url"] . "' width='100' alt='Imagen de soldador'></td>"; // Mostrar la imagen
                            echo "<td>";
                            ...
                            echo "<form method='POST' action='' class='d-inline' onsubmit='return confirm(\"¿Estás seguro que deseas eliminar este soldador?\");'>";
    ...
    <script>
        // Función para mostrar el formulario de agregar nuevo soldador
        ...
        // Función para ocultar el formulario de agregar nuevo soldador
        function ocultarFormularioAgregar() {
            document.getElementById("form-agregar").style.display = "none";
        }

        // Función para mostrar el formulario de edición con los datos del soldador seleccionado
