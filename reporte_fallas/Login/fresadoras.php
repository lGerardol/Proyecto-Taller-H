...
<head>
    ...
    <title>Tabla de Fresadoras</title>
    <!-- Bootstrap CSS -->
...
<body>
    ...
    <div class="container mt-5">
        <h2 class="mb-4 text-center">Lista de Fresadoras</h2>

        <!-- Botón para agregar una nueva fresadora -->
        <div class="text-right">
            <button class="btn btn-primary btn-add" onclick="mostrarFormularioAgregar()">
                <i class="fas fa-plus-circle"></i> Agregar nueva fresadora
            </button>
        </div>

        <!-- Formulario para agregar una nueva fresadora -->
        <div id="form-agregar">
            <h4 class="text-center">Agregar Nueva Fresadora</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-success" name="agregar_fresadora">Agregar Fresadora</button>
                <button type="button" class="btn btn-secondary" onclick="ocultarFormularioAgregar()">Cancelar</button>
            </form>
        </div>

        <!-- Formulario para editar una fresadora (inicialmente oculto) -->
        <div id="form-editar">
            <h4 class="text-center">Editar Fresadora</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-primary" name="actualizar_fresadora">Actualizar Fresadora</button>
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    if (isset($_POST['id_eliminar'])) {
                        ...
                        $sqlEliminar = "DELETE FROM fresadoras WHERE id = ?";
                        $stmtEliminar = $conn->prepare($sqlEliminar);
                        $stmtEliminar->bind_param("i", $idEliminar);
                        
                        if ($stmtEliminar->execute()) {
                            echo "<div class='alert alert-success'>Fresadora eliminada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al eliminar la fresadora: " . $conn->error . "</div>";
                        }
                        $stmtEliminar->close();
                    }

                    // Procesar inserción de una nueva fresadora
                    if (isset($_POST['agregar_fresadora'])) {
                        ...
                        $sqlAgregar = "INSERT INTO fresadoras (nombre, marca, modelo, codigo, fecha_ingreso, estado, observaciones, imagen_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                        ...
                        if ($stmtAgregar->execute()) {
                            echo "<div class='alert alert-success'>Nueva fresadora agregada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al agregar la fresadora: " . $conn->error . "</div>";
                        }
                        $stmtAgregar->close();
                    }

                    // Verificar si se ha enviado la actualización de una fresadora
                    if (isset($_POST['actualizar_fresadora'])) {
                        $id = $_POST['id'];
                        $nombre = $_POST['nombre'];
                        $marca = $_POST['marca'];
                        $modelo = $_POST['modelo'];
                        ...
                        $sqlActualizar = "UPDATE fresadoras SET nombre=?, marca=?, modelo=?, codigo=?, fecha_ingreso=?, estado=?, observaciones=?, imagen_url=? WHERE id=?";
                        ...
                        if ($stmtActualizar->execute()) {
                            echo "<div class='alert alert-success'>Fresadora actualizada correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al actualizar la fresadora: " . $conn->error . "</div>";
                        }
                        $stmtActualizar->close();
                    }

                    // Consulta SQL para obtener las fresadoras
                    $sql = "SELECT * FROM fresadoras";
                    $result = $conn->query($sql);

                    // Mostrar las fresadoras en la tabla
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td><img src='" . $row["imagen_url"] . "' width='100' alt='Imagen de fresadora'></td>"; // Mostrar la imagen
                            echo "<td>";
                            ...
                            echo "<form method='POST' action='' class='d-inline' onsubmit='return confirm(\"¿Estás seguro que deseas eliminar esta fresadora?\");'>";
    ...
    <script>
        // Función para mostrar el formulario de agregar nueva fresadora
        ...
        // Función para ocultar el formulario de agregar nueva fresadora
        function ocultarFormularioAgregar() {
            document.getElementById("form-agregar").style.display = "none";
        }

        // Función para mostrar el formulario de edición con los datos de la fresadora seleccionada
