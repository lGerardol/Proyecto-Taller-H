...
<head>
    ...
    <title>Tabla de Esmeriles</title>
    <!-- Bootstrap CSS -->
...
<body>
    ...
    <nav class="navbar" style="background-color: #dea632; padding: 10px;">
        <div class="container-fluid">
            <div class="navbar-brand">
                <a href="#">
                    <img src="https://www.itca.edu.sv/wp-content/uploads/2017/01/logo-new-slider-1.png" alt="Logo" style="max-height: 50px;"> <!-- Ajuste de tamaño del logo -->
                </a>
            </div>
        </div>
    </nav>
    ...
    <div class="container mt-5">
        <h2 class="mb-4 text-center">Lista de Esmeriles</h2>

        <!-- Botón para agregar un nuevo esmeril -->
        <div class="text-right">
            <button class="btn btn-primary btn-add" onclick="mostrarFormularioAgregar()">
                <i class="fas fa-plus-circle"></i> Agregar nuevo esmeril
            </button>
        </div>

        <!-- Formulario para agregar un nuevo esmeril -->
        <div id="form-agregar">
            <h4 class="text-center">Agregar Nuevo Esmeril</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-success" name="agregar_esmeril">Agregar Esmeril</button>
                <button type="button" class="btn btn-secondary" onclick="ocultarFormularioAgregar()">Cancelar</button>
            </form>
        </div>

        <!-- Formulario para editar un esmeril (inicialmente oculto) -->
        <div id="form-editar">
            <h4 class="text-center">Editar Esmeril</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-primary" name="actualizar_esmeril">Actualizar Esmeril</button>
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    if (isset($_POST['id_eliminar'])) {
                        ...
                        $sqlEliminar = "DELETE FROM esmeriles WHERE id = ?";
                        $stmtEliminar = $conn->prepare($sqlEliminar);
                        $stmtEliminar->bind_param("i", $idEliminar);
                        
                        if ($stmtEliminar->execute()) {
                            echo "<div class='alert alert-success'>Esmeril eliminado correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al eliminar el esmeril: " . $conn->error . "</div>";
                        }
                        $stmtEliminar->close();
                    }

                    // Procesar inserción de un nuevo esmeril
                    if (isset($_POST['agregar_esmeril'])) {
                        ...
                        $sqlAgregar = "INSERT INTO esmeriles (nombre, marca, modelo, codigo, fecha_ingreso, estado, observaciones, imagen_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                        ...
                        if ($stmtAgregar->execute()) {
                            echo "<div class='alert alert-success'>Nuevo esmeril agregado correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al agregar el esmeril: " . $conn->error . "</div>";
                        }
                        $stmtAgregar->close();
                    }

                    // Verificar si se ha enviado la actualización de un esmeril
                    if (isset($_POST['actualizar_esmeril'])) {
                        $id = $_POST['id'];
                        $nombre = $_POST['nombre'];
                        $marca = $_POST['marca'];
                        $modelo = $_POST['modelo'];
                        ...
                        $sqlActualizar = "UPDATE esmeriles SET nombre=?, marca=?, modelo=?, codigo=?, fecha_ingreso=?, estado=?, observaciones=?, imagen_url=? WHERE id=?";
                        ...
                        if ($stmtActualizar->execute()) {
                            echo "<div class='alert alert-success'>Esmeril actualizado correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al actualizar el esmeril: " . $conn->error . "</div>";
                        }
                        $stmtActualizar->close();
                    }

                    // Consulta SQL para obtener los esmeriles
                    $sql = "SELECT * FROM esmeriles";
                    $result = $conn->query($sql);

                    // Mostrar los esmeriles en la tabla
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td><img src='" . $row["imagen_url"] . "' width='100' alt='Imagen de esmeril'></td>"; // Mostrar la imagen
                            echo "<td>";
                            ...
                            echo "<form method='POST' action='' class='d-inline' onsubmit='return confirm(\"¿Estás seguro que deseas eliminar este esmeril?\");'>";
    ...
    <script>
        // Función para mostrar el formulario de agregar nuevo esmeril
        ...
        // Función para ocultar el formulario de agregar nuevo esmeril
        function ocultarFormularioAgregar() {
            document.getElementById("form-agregar").style.display = "none";
        }

        // Función para mostrar el formulario de edición con los datos del esmeril seleccionado
