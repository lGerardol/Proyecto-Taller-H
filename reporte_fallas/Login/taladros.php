...
<body>
    ...
    <div class="container mt-5">
        <h2 class="mb-4 text-center">Lista de Taladros</h2>

        <!-- Botón para agregar un nuevo taladro -->
        <div class="text-right">
            <button class="btn btn-primary btn-add" onclick="mostrarFormularioAgregar()">
                <i class="fas fa-plus-circle"></i> Agregar nuevo taladro
            </button>
        </div>

        <!-- Formulario para agregar un nuevo taladro -->
        <div id="form-agregar">
            <h4 class="text-center">Agregar Nuevo Taladro</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-success" name="agregar_taladro">Agregar Taladro</button>
                <button type="button" class="btn btn-secondary" onclick="ocultarFormularioAgregar()">Cancelar</button>
            </form>
        </div>

        <!-- Formulario para editar un taladro (inicialmente oculto) -->
        <div id="form-editar">
            <h4 class="text-center">Editar Taladro</h4>
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-primary" name="actualizar_taladro">Actualizar Taladro</button>
            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    if (isset($_POST['id_eliminar'])) {
                        ...
                        $sqlEliminar = "DELETE FROM taladros WHERE id = ?";
                        $stmtEliminar = $conn->prepare($sqlEliminar);
                        $stmtEliminar->bind_param("i", $idEliminar);
                        
                        if ($stmtEliminar->execute()) {
                            echo "<div class='alert alert-success'>Taladro eliminado correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al eliminar el taladro: " . $conn->error . "</div>";
                        }
                        $stmtEliminar->close();
                    }

                    // Procesar inserción de un nuevo taladro
                    if (isset($_POST['agregar_taladro'])) {
                        ...
                        $sqlAgregar = "INSERT INTO taladros (nombre, marca, modelo, codigo, fecha_ingreso, estado, observaciones, imagen_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                        ...
                        if ($stmtAgregar->execute()) {
                            echo "<div class='alert alert-success'>Nuevo taladro agregado correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al agregar el taladro: " . $conn->error . "</div>";
                        }
                        $stmtAgregar->close();
                    }

                    // Verificar si se ha enviado la actualización de un taladro
                    if (isset($_POST['actualizar_taladro'])) {
                        $id = $_POST['id'];
                        $nombre = $_POST['nombre'];
                        $marca = $_POST['marca'];
                        $modelo = $_POST['modelo'];
                        ...
                        $sqlActualizar = "UPDATE taladros SET nombre=?, marca=?, modelo=?, codigo=?, fecha_ingreso=?, estado=?, observaciones=?, imagen_url=? WHERE id=?";
                        ...
                        if ($stmtActualizar->execute()) {
                            echo "<div class='alert alert-success'>Taladro actualizado correctamente</div>";
                        } else {
                            echo "<div class='alert alert-danger'>Error al actualizar el taladro: " . $conn->error . "</div>";
                        }
                        $stmtActualizar->close();
                    }

                    // Consulta SQL para obtener los taladros
                    $sql = "SELECT * FROM taladros";
                    $result = $conn->query($sql);

                    // Mostrar los taladros en la tabla
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td><img src='" . $row["imagen_url"] . "' width='100' alt='Imagen de taladro'></td>"; // Mostrar la imagen
                            echo "<td>";
                            ...
                            echo "<form method='POST' action='' class='d-inline' onsubmit='return confirm(\"¿Estás seguro que deseas eliminar este taladro?\");'>";
    ...
    <script>
        // Función para mostrar el formulario de agregar nuevo taladro
        ...
        // Función para ocultar el formulario de agregar nuevo taladro
        function ocultarFormularioAgregar() {
            document.getElementById("form-agregar").style.display = "none";
        }

        // Función para mostrar el formulario de edición con los datos del taladro seleccionado
